...
export class DocumentQueryService {
   ...
   * Node lookup -> O(1)
   */
  static async getNode(id: string): Promise<QueryResult<NodeResult | null>> {
   ...
   */
  static async getChildren(parentId: string | null, limit: number, cursor?: string): Promise<QueryResult<NodeResult[]>> {
    const start = Date.now();
   ...
   */
  static async getAncestors(nodeId: string): Promise<QueryResult<NodeResult[]>> {
    const start = Date.now();
    
    // Using Drizzle's execute with raw SQL for recursive CTE
    const query = sql`
      WITH RECURSIVE ancestors AS (
        SELECT * FROM document_nodes WHERE id = ${nodeId}
        UNION ALL
   ...
   */
  static async getDescendants(nodeId: string, depthLimit: number = 10): Promise<QueryResult<NodeResult[]>> {
    const start = Date.now();
    
    const query = sql`
      WITH RECURSIVE descendants AS (
        SELECT *, 1 as depth FROM document_nodes WHERE id = ${nodeId}
        UNION ALL
        SELECT d.*, a.depth + 1 FROM document_nodes d
        INNER JOIN descendants a ON d.parent_id = a.id
   ...
   */
  static async getWindow(parentId: string | null, cursor: string, windowSize: number): Promise<QueryResult<NodeResult[]>> {
    return this.getChildren(parentId, windowSize, cursor);
  }

  /**
   * Context expansion -> Siblings before and after
   */
  static async expandContext(nodeId: string, before: number, after: number): Promise<QueryResult<NodeResult[]>> {
    const start = Date.now();
    
    // 1. Get the target node to find its parent and lexoRank
    const targetNodeResult = await this.getNode(nodeId);
    ...
    const beforeConditions = target.parentId === null 
      ? [isNull(documentNodes.parentId), lt(documentNodes.lexoRank, target.lexoRank)]
      : [eq(documentNodes.parentId, target.parentId), lt(documentNodes.lexoRank, target.lexoRank)];
    ...
    const afterConditions = target.parentId === null 
      ? [isNull(documentNodes.parentId), gt(documentNodes.lexoRank, target.lexoRank)]
      : [eq(documentNodes.parentId, target.parentId), gt(documentNodes.lexoRank, target.lexoRank)];
   ...
   */
  static async getHeadingTree(documentId: string): Promise<QueryResult<NodeResult[]>> {
    const start = Date.now();
    
    const result = await db.select()
      .from(documentNodes)
      .where(and(
        eq(documentNodes.fileId, documentId),
