...
const credentialUriPattern = /\b(?:postgres(?:ql)?|mysql|mariadb|mongodb(?:\+srv)?|redis|rediss|amqp|https?):\/\/([^/\s:@]+):([^@\s/]+)@(\[[^\]]+\]|[^/\s:?#]+)/gi;
...
function isDemonstrablySyntheticLoopbackUri(match) {
  ...
  const commonPlaceholder = /^(?:password|postgres|test|example|changeme|dummy)$/i.test(password);
...
function inspectText(file, content) {
  ...
  for (const [category, pattern] of patterns) {
    ...
    for (const match of content.matchAll(pattern)) {
      if (category === 'credential-bearing URI' && isDemonstrablySyntheticLoopbackUri(match)) {
        continue;
      }
...
function looksLikeBomlessUtf16(sample) {
  ...
  const littleEndianShape = oddNuls / oddCount > 0.3 && evenPrintable / evenCount > 0.6;
  const bigEndianShape = evenNuls / evenCount > 0.3 && oddPrintable / oddCount > 0.6;
...
function classifyBytes(sample, knownText) {
  ...
  if (knownBinarySignatures.some(signature =>
    sample.length >= signature.length && sample.subarray(0, signature.length).equals(signature)
  )) {
    return { kind: 'binary' };
  }
...
function scanRepository(repositoryRoot = process.cwd()) {
  ...
  for (const entry of entries) {
    if (entry.stage !== 0) {
      findings.push({ file: entry.file, line: 0, category: 'unmerged tracked entry' });
      continue;
    }
    ...
    if (FORBIDDEN_TRACKED_ROOT_PATHS.has(entry.file)) {
      findings.push({ file: entry.file, line: 0, category: 'forbidden tracked root residue' });
      continue;
    }
    ...
    if (!isWithinRepository(root, absoluteFile)) {
      findings.push({ file: entry.file, line: 0, category: 'tracked path escapes repository root' });
      continue;
    }
    ...
    try {
      ...
      findings.push({ file: entry.file, line: 0, category: 'tracked file is unreadable' });
      continue;
    }

    if (!stats.isFile()) {
      findings.push({ file: entry.file, line: 0, category: 'tracked regular-file mode is not a regular file' });
      continue;
    }

    let classification;
    try {
      classification = classifyBytes(readSample(absoluteFile, stats.size), isKnownTextFile(entry.file));
    } catch {
      findings.push({ file: entry.file, line: 0, category: 'tracked file sample is unreadable' });
      continue;
    }
    ...
    if (classification.kind === 'ambiguous') {
      findings.push({ file: entry.file, line: 0, category: `ambiguous text encoding: ${classification.reason}` });
      continue;
    }

    if (stats.size > MAX_TEXT_BYTES) {
      findings.push({ file: entry.file, line: 0, category: 'tracked text file exceeds scan size limit' });
      continue;
    }

    try {
      const content = decodeTrackedText(readFileSync(absoluteFile), classification);
      scannedTextFiles += 1;
      findings.push(...inspectText(entry.file, content));
    } catch {
      findings.push({ file: entry.file, line: 0, category: 'tracked text file cannot be decoded safely' });
    }
  }
...
function runCli(repositoryRoot = process.cwd()) {
  ...
  try {
    ...
    console.error('Committed-secret guard failed closed while enumerating tracked files.');
    return 1;
  }
  ...
  if (result.findings.length > 0) {
    ...
    for (const finding of result.findings.slice(0, MAX_REPORTED_FINDINGS)) {
      ...
      console.error(`${JSON.stringify(finding.file)}${location} [${finding.category}]`);
    }
    if (result.findings.length > MAX_REPORTED_FINDINGS) {
      console.error(`Additional findings suppressed: ${result.findings.length - MAX_REPORTED_FINDINGS}`);
    }
    return 1;
  }
