...
function withRepository(files, callback) {
  const repository = mkdtempSync(
    path.join(os.tmpdir(), "studyai-secret-guard-"),
  );

  try {
    execFileSync("git", ["init", "--quiet"], { cwd: repository });
    execFileSync("git", ["config", "user.email", "guard@example.invalid"], {
      cwd: repository,
    });
    execFileSync("git", ["config", "user.name", "StudyAI Guard Test"], {
      cwd: repository,
    });
...
test("rejects a UTF-8 credential in an unexpected extension", () => {
  assertRejected(
    { "payload.custom": syntheticCredentialUri },
...
test("rejects a UTF-16BE credential", () => {
  assertRejected(
    { "encoded.data": utf16BigEndian(syntheticCredentialUri) },
    "credential-bearing URI",
  );
});
...
test("scans extensionless text files", () => {
  assertRejected(
    { credentials: syntheticCredentialUri },
    "credential-bearing URI",
  );
});
...
test("rejects the exact forbidden root residue path", () => {
  assertRejected(
    { "patch.tmp": "safe content" },
    "forbidden tracked root residue",
  );
});
